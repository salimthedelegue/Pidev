...
import com.example.stockfx.entities.SmsApi;
...
import com.example.stockfx.entities.pdfsalim;
  ...
  stage.show();
    ...
    @FXML
     private void PDF(ActionEvent event) throws DocumentException, BadElementException, IOException, FileNotFoundException, InterruptedException, SQLException {
        pdfsalim pd=new pdfsalim();
        try{
        pd.GeneratePdf("list of reclamations");
    ...
    private void exel(ActionEvent event) throws IOException {
        ...
        file = new File("C:\\Users\\Sana\\Downloads\\v2withtableview\\Stockfx\\src\\main\\java\\com\\example\\stockfx\\GUI\\reclamation.xlsx");
        if(file.exists()) desktop.open(file);
              ServicesReclamation su = new ServicesReclamation() ;
