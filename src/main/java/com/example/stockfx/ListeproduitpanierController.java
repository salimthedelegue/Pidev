...
public class ListeproduitpanierController implements Initializable {
 ...
 if(p.verifpaniervide(us.getConnected().getId_user())==false){
    ...
    public void liste(){
       ...
       List<Produit> produits=ps.afficherproduitpanier(us.getConnected().getId_user());
       
       for(int i=0;i<produits.size();i++){
           System.out.println(ps.afficherproduitpanier(us.getConnected().getId_user()).get(i).getId_produit());
           int j=i;
           HBox h1=new HBox();
        ...
        supprimer.setOnAction( new EventHandler<ActionEvent>(){
            @Override public void handle(ActionEvent e){
                        ...
                        pans.supprimerproduitpanier(us.getConnected().getId_user(),produits.get(j).getId_produit());
                        liste();
                         PanierService p=new PanierService();
    ...
    private void comfirmercommande(ActionEvent event) {
        ...
        comm.comfirmercommande(us.getConnected().getId_user());
        liste();
        id_commande.setVisible(false);
    }
